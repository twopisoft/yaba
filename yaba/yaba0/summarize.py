...
def u(s):
    ...
    if _IS_PYTHON_3:
        ...
        ss=filter(lambda x: x in string.printable,s)
        return codecs.unicode_escape_decode(ss)[0]
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        summary=str(summarize_page(sys.argv[1]))
        print("summary=",summary,"length=",len(summary))
        exit(0)
